...
class Chart {
  ...
  constructor(options) {
    ...
    /**
     * Custom min value(y axis), if not provided, it will be caculated based on data
     * @type {Number}
     */
    this._yMin = options.yMin;

    /**
     * Custom max value(y axis), if not provided, it will be caculated based on data
     * @type {Number}
     */
 ...
 */
  ...
  computeDataProperty() {
    ...
    this.yMin = defined(this._yMin) ? this._yMin : this.calculateYMin();
    this.yMax = defined(this._yMax) ? this._yMax : this.calculateYMax();
  }
