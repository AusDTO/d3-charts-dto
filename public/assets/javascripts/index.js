...
var _formatData = require('../Helpers/formatData');

var _formatData2 = _interopRequireDefault(_formatData);

var _getDate = require('../Helpers/getDate');
...
var Legend = function () {
  function Legend(options) {
    ...
    this.data = this.chart.type === 'bar' ? _lodash2.default.reverse(this.chart.transformedData) : this.chart.transformedData;
  ...
  Legend.prototype.render = function render() {
    ...
    this.date = this.container.append('div').attr('class', 'date').datum(this.data[0]);

    var row = this.container.append('table').selectAll('tr').data(this.data).enter().append('tr');

    this.th = row.append('th');

    var svg = this.th.append('svg').attr('width', iconSize).attr('height', iconSize).append('g').attr('class', 'legend--icon');

    svg.append('rect').attr('x', 0).attr('y', 0).attr('width', iconSize).attr('height', iconSize).attr('fill', function (d) {
      return (0, _defined2.default)(d[0]) ? d[0].color : d.data.color;
    });

    svg.append('line').attr('x1', 0).attr('x2', iconSize).attr('y1', iconSize / 2).attr('y2', iconSize / 2).attr('stroke-linecap', 'butt').attr('stroke', function (d) {
      return (0, _defined2.default)(d[0]) ? d[0].color : d.data.color;
    }).attr('stroke-dasharray', function (d) {
      return (0, _defined2.default)(d[0]) ? d[0].altLineStyle : 0;
    });
    ...
    if (this.type === 'pie') {
      this.td.text(function (d) {
        return (0, _formatData2.default)(d.data.y, _this.chart.prefix, _this.chart.suffix, _this.displayRoundedData);
      });
    }
  };
  ...
  Legend.prototype.updateLegendIcon = function updateLegendIcon() {
    var that = this;
    var squares = this.th.selectAll('.legend--icon rect');
    var lines = this.th.selectAll('.legend--icon line');

    squares.each(function () {
      _d2.default.select(this).attr('fill', function (d) {
        return (0, _defined2.default)(d[0]) ? that.chart.isHighContrastMode ? d[0].altColor : d[0].color : that.chart.isHighContrastMode ? d.data.altColor : d.data.color;
      }).attr('visibility', that.chart.type === 'line' && that.chart.isHighContrastMode === true ? 'hidden' : 'visible').attr('rx', that.chart.isHighContrastMode ? 2 : iconSize / 2).attr('ry', that.chart.isHighContrastMode ? 2 : iconSize / 2);
    });

    lines.each(function () {
      _d2.default.select(this).attr('visibility', that.chart.type === 'line' && that.chart.isHighContrastMode === true ? 'visible' : 'hidden');
    });
  };
  ...
  Legend.prototype.hover = function hover(i) {
    ...
    this.date.text(function (d) {
      return (0, _getDate2.default)().long(d[i].x);
    });
    this.td.text(function (d) {
      return (0, _formatData2.default)(d[i].y, _this2.chart.prefix, _this2.chart.suffix, _this2.displayRoundedData);
    });
  };
...
},{"../Helpers/defined":14,"../Helpers/formatData":15,"../Helpers/getDate":17,"d3":19,"lodash":21}],7:[function(require,module,exports){
'use strict';
...
},{"../Helpers/isActive":18,"./BarChart":3,"d3":19,"textures":22}],12:[function(require,module,exports){
'use strict';
...
var XAxis = function (_Axis) {
  ...
  function XAxis(options) {
    ...
    var _this = _possibleConstructorReturn(this, _Axis.call(this, options));
  ...
  XAxis.prototype.render = function render() {
    ...
    _Axis.prototype.render.call(this);
    this.axis.attr('class', 'x axis').attr('transform', 'translate(' + this.chart.margin.left + ', ' + (this.chart.height + this.chart.margin.top) + ')');

    numberOfInstance++;
    _d2.default.select(window).on('resize.' + 'xAxis' + numberOfInstance, function () {
      _this2.axis.call(_this2.Axis);
    });
  };
...
var YAxis = function (_Axis) {
  ...
  YAxis.prototype.render = function render() {
    _Axis.prototype.render.call(this);
    this.axis.attr('class', 'y axis').attr('transform', 'translate(7, ' + this.chart.margin.top + ')').selectAll('text').attr('y', -6).style('text-anchor', 'start');
  };
  ...
  _createClass(YAxis, [{
    ...
    get: function get() {
      ...
      var min = this.chart.yMin;
      var max = this.chart.yMax;
...
},{"./formatSeconds":16,"d3":19,"defined":20}],16:[function(require,module,exports){
'use strict';
...
},{"./textures.js":23}],23:[function(require,module,exports){
"use strict";
  ...
  umd(function () {
    return {
      ...
      lines: function lines() {
        ...
        shapeRendering = "auto";
        path = function path(orientation) {
          switch (orientation) {
            case "0/8":
              return function (s) {
